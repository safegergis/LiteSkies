...
export default function FetchWeather({ first, zip }) {
  ...
  if (first) {
    ...
    useEffect(
      () => {
        navigator.geolocation.getCurrentPosition(
          ...
          (pos) => {
            //closure on successful location grab
            const locationData = {
          ...
          (error) => {
            //closure on unsuccessful location grab
            alert("Location is unable to be found");
          }
        );
      },
      [] /* run on mount only because of empty dependency array */
    );
    ...
    useEffect(
      ...
      [
        location,
      ] /*this causes useeffect to only run when location state changes */
    );
  } else {
    ...
    const fetchWeather = async () => {
      try {
        const fetchWeather = await fetch(
          ...
          .then((res) => res.json()) // process json
          .then((data) => setWeatherData(data));
      } catch (error) {
        console.error("Error fetching data:", error);
      }
    ...
    useEffect(
      ...
      [] /*only runs when component mounts because of empty depenecy array*/
    );
  }
  if (weatherData === null) {
    return (
